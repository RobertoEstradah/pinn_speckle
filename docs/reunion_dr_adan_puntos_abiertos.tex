...
\noindent
\textbf{Los puntos 1 y 2 ya se resolvieron} en la conversación del 18 de
septiembre y están aplicados: el título y el objetivo general. Los tres
restantes siguen abiertos y se presentan aquí con su texto actual, lo que
muestran los resultados, y la decisión concreta que hay que tomar.
...
\section{El objetivo general \normalfont\textit{(resuelto el 18/09)}}

\subsection{Texto anterior (Cap. 1, \textit{Objetivo general})}
...
\cita{\ldots para la simulación \textbf{físicamente precisa} de patrones de
speckle óptico, mediante una \textbf{formulación modal} de la ecuación de
Helmholtz bidimensional con campo eléctrico complejo, en la que la condición de
Cauchy se impone por construcción.}

\noindent
\textbf{Aplicado.} Sale también «acelerada», por la misma razón que del título.
Se retiró además el adjetivo «directa» de la segunda contribución (Cap.~1), donde
...
2 & Objetivo general & \textit{Resuelto el 18/09: aplicado} \\
...
\vspace{0.6em}
\noindent
El \textbf{punto 4 condiciona al 3}: no se puede decidir sobre NB04 sin resolver
