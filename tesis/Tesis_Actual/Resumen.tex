...
La validación se extiende a $z=2\lambda$ con una PINN-SIREN de dominio único
por pantalla
($L^2=0.993\%$) y, mediante descomposición del dominio en redes locales de
$1\lambda$ acopladas por condiciones de Cauchy duras, hasta $z=5\lambda$
($L^2=2.244\%$ al final del recorrido y un máximo de $3.507\%$ sobre $201$
planos) y hasta $z=10\lambda$ con diez redes ($L^2=2.617\%$ y un máximo de
$3.726\%$), en todos los casos sobre cinco realizaciones de fase conocidas.
Una cadena de veinte bloques hasta $z=20\lambda$, ensayada sobre una única
pantalla y sin refinamiento, no alcanza el criterio ($L^2=8.743\%$).
Los protocolos de optimización difieren entre experimentos y no deben
generalizarse: las validaciones 1D y 2D emplean Adam seguido de L-BFGS con
muestreo por hipercubo latino (LHS); las formulaciones modales de dominio único
en $z=1\lambda$ y $z=2\lambda$ usan sólo Adam y sin LHS, porque su pérdida
contiene únicamente el residuo modal; y la cadena de $z=5\lambda$ entrena o
refina cada bloque con Adam seguido de L-BFGS. La reproducibilidad frente a la
