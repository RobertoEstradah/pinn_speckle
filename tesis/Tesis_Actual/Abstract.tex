...
The validation extends to $z=2\lambda$ with the same single-domain network
...
The 1D and 2D validations use a two-phase Adam~+~L-BFGS optimization strategy
with Latin Hypercube Sampling (LHS); the modal formulation of the speckle uses a
different protocol, with Adam only and no LHS, because its loss contains the
modal residual alone. Robustness to initialization was verified in the 2D case
