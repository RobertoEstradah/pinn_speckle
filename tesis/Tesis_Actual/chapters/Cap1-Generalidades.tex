...
\begin{enumerate}
    ...
    \item Generar y validar estadísticamente el patrón de \textit{speckle} óptico
    a distancias físicas reales (NB03), conforme al criterio de Goodman: contraste
    $C = \sigma_I/\langle I\rangle$ y ajuste a la distribución exponencial negativa
    (test de Kolmogorov-Smirnov). Se exploró adicionalmente la generación de
    \textit{speckle} resolviendo Helmholtz por PINN con una condición de frontera
    de fase aleatoria; a la escala de dominio validada en NB01/NB02 ($k=2\pi$, una
    longitud de onda) este enfoque no logra desarrollar \textit{speckle}
    estadísticamente válido, por lo que la validación de esta tesis se apoya en la
    generación estadística directa (véase el Capítulo~\ref{cap3:modelo}).
...
\begin{description}
    ...
    Se genera el patrón de \textit{speckle} a 10 distancias físicas reales
    (2--20 cm) mediante generación estadística directa: ruido blanco gaussiano
    complejo filtrado en frecuencia, con el ancho del filtro fijado por el tamaño
    de grano teórico de Goodman $\text{grano} \approx \lambda z/D$. La validación
    estadística sigue el criterio de Goodman \parencite{goodman2007speckle}:
    contraste $C = \sigma_I / \langle I \rangle$ y test de Kolmogorov-Smirnov
    contra la distribución exponencial teórica, evaluado sobre una muestra
    decorrelacionada al tamaño de grano para evitar violar el supuesto de
    independencia del test. Se exploró un enfoque alternativo resolviendo
    Helmholtz por PINN con frontera de fase aleatoria; a la escala de dominio
    heredada de NB02 este enfoque no desarrolla \textit{speckle}
    estadísticamente válido (Sección~\ref{cap3:nb03}).
