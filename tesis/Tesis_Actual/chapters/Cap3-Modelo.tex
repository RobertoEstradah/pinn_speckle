...
La ecuación de Helmholtz 2D en $\Omega = [0,1]^2$ con condición de frontera
rugosa en $y = 0$ y bordes libres en $x = 0$, $x = 1$ e $y = 1$:
%
\begin{equation}
...
\end{equation}
%
donde $\psi(x)$ se muestrea en $N_\psi = 256$ puntos equiespaciados. Esta
condición modela una pantalla de fase aleatoria cuyo rango de variación abarca
$\Delta\psi \gg 2\pi$, condición suficiente para \textit{speckle} completamente
desarrollado \parencite{goodman2007speckle}.

% ──────────────────────────────────────────────────────────────────────────────
\section{Arquitectura SIREN}

Se emplea una arquitectura SIREN con los parámetros consolidados a partir de la
validación 1D (NB01) y escalados para el caso 2D (NB02, NB03):
...
\begin{itemize}
    ...
    \item \textbf{Parámetros totales:} $16{,}833$ (NB01) y $66{,}690$ (NB02/NB03).
\end{itemize}
...
$\mathcal{L}_\text{datos}$. NB03 sigue un esquema distinto, descrito en la
Sección~\ref{cap3:nb03}: solo
...
\midrule
Tasa de aprendizaje Adam ($\eta$) & $10^{-3}$ & NB01, NB02, NB03 \\
Épocas máximas Adam             & 15{,}000  & NB01, NB02, NB03 \\
Paciencia (\textit{early stopping}) & 800 épocas & NB02, NB03 \\
$\delta$ mejora mínima          & $10^{-7}$ & NB02, NB03 \\
Iteraciones máximas L-BFGS      & 500 (NB01), 1{,}000 (NB02/03) & - \\
Historial L-BFGS                & 50 (NB01), 100 (NB02/03) & - \\
Semilla global (SEED)           & 42        & NB01, NB02, NB03 \\
Peso de física $\lambda_\text{fís}$ & 1.0 (NB01), 0.1 (NB02/03) & - \\
\bottomrule
\end{tabular}
...
Para la validación del \textit{speckle}:
\begin{itemize}
    ...
    \exp(-I/\langle I \rangle)$. Criterio formal: $p > 0.05$.
    \item \textbf{Fracción de colas:} Fracción de píxeles con $I > 2\langle I \rangle$,
    comparada con el valor teórico $e^{-2} \approx 0.1353$.
\end{itemize}
