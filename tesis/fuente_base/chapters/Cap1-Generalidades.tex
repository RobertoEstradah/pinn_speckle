...
\begin{itemize}
    ...
    frontera con fase aleatoria $\phi(x) \sim \mathcal{U}(0, 2\pi)$.
...
\begin{itemize}
    ...
    una frontera con fase aleatoria discreta ($N_\phi = 256$ puntos). No se
...
\begin{description}
    ...
    \item[\textbf{Fase 3 - NB03: Simulación de \textit{speckle} óptico (trabajo futuro).}]
    Se aplicará el modelo validado a la generación de \textit{speckle}, reemplazando
    la condición de onda plana por una frontera con fase aleatoria
    $\phi(x) \sim \mathcal{U}(0, 2\pi)$ en $y = 0$ y bordes libres en el
    resto del dominio. La validación estadística seguirá el criterio de Goodman
