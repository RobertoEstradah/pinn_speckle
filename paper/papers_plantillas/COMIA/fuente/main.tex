% ============================================================
%  Validacion de PINN-SIREN para Helmholtz 1D y 2D — Version COMIA/LNCS
...
\title{Validaci\'{o}n de Redes Neuronales Informadas por F\'{i}sica con
       activaci\'{o}n sinusoidal para la ecuaci\'{o}n de Helmholtz 1D y 2D
       con campo complejo%
...
\titlerunning{PINN-SIREN para la ecuaci\'{o}n de Helmholtz 1D y 2D}
...
analytical solutions, several orders of magnitude below the $5\%$
engineering threshold adopted in this work.
A two-phase Adam\,+\,L-BFGS optimization strategy and Latin Hypercube
Sampling (LHS) are key to convergence and accuracy.
Multi-seed validation ($\mathrm{SEED}\in\{42,123,777\}$) yields a mean
$L^2$ error of $0.192\%$ with a population standard deviation of
$0.089\%$, that is, a coefficient of variation of approximately $46\%$
about the mean: the spread across initialisations is considerable and
must be taken into account when reading any individual value.
...
$\approx\!1.3\,\mathrm{ms}$ on an NVIDIA RTX~5050 GPU. No comparison
against a finite-element solver is reported here, so no speed-up factor
is claimed: that benchmark is left as future work.
...
(\textit{mesh-free}) orientado a la simulaci\'{o}n de
...
caso 2D respecto a soluciones anal\'{i}ticas, varios \'{o}rdenes de
magnitud por debajo del umbral ingenieril de $5\%$ adoptado en este
trabajo.
La validaci\'{o}n multi-semilla ($\mathrm{SEED}\in\{42,123,777\}$)
arroja un error $L^2$ promedio de $0.192 \pm 0.089\%$, es decir un
coeficiente de variaci\'{o}n del $46\%$ respecto a la media: la
dispersi\'{o}n frente a la inicializaci\'{o}n es considerable y debe
tenerse en cuenta al interpretar cualquier valor individual.
