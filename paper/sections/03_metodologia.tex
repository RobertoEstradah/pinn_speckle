...
\paragraph{Speckle óptico.}
La ecuación de Helmholtz 2D en $\Omega = [0,1]^2$ con condición de frontera rugosa
en $y = 0$ y bordes libres en $x = 0$, $x = 1$, $y = 1$:
%
\begin{equation}
...
\end{equation}
%
donde $\phi(x)$ es muestreada en $N_\phi = 256$ puntos equiespaciados.
Esta condición modela una pantalla de fase aleatoria cuya desviación estándar cumple
$\sigma_\phi > 2\pi$, condición suficiente para speckle completamente desarrollado
\citep{Goodman2007_Speckle, Fang2020_scattering}.
A diferencia de los métodos basados en malla \citep{Jin2014_FEM}, el enfoque PINN permite
resolver el campo en el interior del dominio sin construir una malla espacial,
reduciendo drásticamente el costo de almacenamiento.
...
Esta combinación, descrita en \citet{Wang2021_failurePINNs}, supera a Adam puro en
precisión final y a L-BFGS puro en robustez ante mínimos locales.
...
El umbral de la tesis es $\varepsilon_{L2} < 5\%$.

\paragraph{Contraste y test KS} (NB03):
El contraste $C = \sigma_I / \langle I \rangle$ debe satisfacer $|C - 1| < 0.1$,
y el test de Kolmogorov-Smirnov contra la distribución exponencial teórica debe
arrojar $p > 0.05$.
